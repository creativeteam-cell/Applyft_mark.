...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // -ss ДО -i (быстрый seek к ключевому кадру) + -t ДЛИТЕЛЬНОСТЬ (не -to, оно
    // неоднозначно как output-опция и ломало длину кусков). Перекодирование +
    // setpts + постоянный fps=30 убирают "стоп-кадр" в начале и микрофризы на стыках.
    const durSec = (typeof startMs === 'number' && typeof endMs === 'number')
      ? Math.max(0.1, (endMs - startMs) / 1000) : undefined
    const args = ['-y']
    if (typeof startMs === 'number') args.push('-ss', (startMs / 1000).toFixed(3))
    args.push('-i', inPath)
    if (durSec) args.push('-t', durSec.toFixed(3))
    args.push(
