...
export async function POST(req: NextRequest) {
  ...
  const { model_name, prompt, first_frame, duration, resolution } = await req.json()
  if (!prompt?.trim() && !first_frame) return NextResponse.json({ error: 'Prompt or image required' }, { status: 400 })

  try {
    const result = await createTurboVideoTask({ model_name, prompt, first_frame, duration, resolution })
