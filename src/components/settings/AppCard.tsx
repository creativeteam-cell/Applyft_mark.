...
export function AppCard({ app, expanded, onExpand, onSave, onDelete }: AppCardProps) {
  ...
  useEffect(() => {
    if (expanded) {
      fetch(`/api/concepts?app=${app.code}`)
        .then(r => r.json())
          ...
          <div className="flex border-b" style={{ borderColor: 'var(--border)' }}>
            {(['info', 'library'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                ...
                {tab === 'info' ? 'Info' : `Creative Library ${concepts.length > 0 ? `(${concepts.length})` : ''}`}
              </button>
            ))}
          </div>
          ...
          <div className="px-6 pb-6 pt-5">
            ...
            {activeTab === 'library' && (
              <CreativeLibrary
                ...
                onConceptDeleted={(id) => setConcepts(prev => prev.filter(c => c.id !== id))}
              />
            )}
          </div>
        </div>
