...
function VideoCardModal({ item, onClose, onRefresh, onSelectItem, onContinue }: { item: VideoItem; onClose: () => void; onRefresh: () => void; onSelectItem?: (item: VideoItem) => void; onContinue?: (params: { sourceItem: VideoItem; prompt: string; model: 'kling-video-o1' | 'kling-v3-omni'; duration: number }) => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6"
      ...
      <div className="relative flex rounded-2xl overflow-hidden max-h-[90vh] w-full max-w-3xl"
        ...
        <div className="flex flex-col flex-1 min-w-0 overflow-y-auto p-5">
          ...
          {item.sourceVideoId && onSelectItem && (
            <div className="mb-4">
              <button onClick={() => {
                const sourceItem = history.find(v => v.id === item.sourceVideoId)
                if (sourceItem) onSelectItem(sourceItem)
              }}
