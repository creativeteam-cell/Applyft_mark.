...
interface VideoItem {
  ...
  targetLang?: string | null     // код языка, если это перевод
}
...
// ── ProcessingVideoCard — карточка генерящегося видео (спиннер + таймер) ─────

function ProcessingVideoCard({ pv, onDismiss }: { pv: ProcessingVideo; onDismiss: () => void }) {
  const [elapsed, setElapsed] = useState(Math.floor((Date.now() - pv.startedAt) / 1000))
  useEffect(() => {
    if (pv.status !== 'processing') return
    const t = setInterval(() => setElapsed(Math.floor((Date.now() - pv.startedAt) / 1000)), 1000)
    return () => clearInterval(t)
  }, [pv.startedAt, pv.status])
  ...
  return (
    <div className="rounded-xl overflow-hidden relative flex flex-col"
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 text-center">
        {isError ? (
          <>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#f87171" strokeWidth="2" className="mb-2">
              <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
            </svg>
            <p className="text-xs font-medium mb-1" style={{ color: '#f87171' }}>Failed</p>
            <p className="text-[10px] line-clamp-3" style={{ color: 'var(--text-muted)' }}>{pv.error}</p>
            <button onClick={onDismiss} className="mt-2 text-[10px] px-2 py-1 rounded-lg" style={{ background: 'rgba(255,255,255,0.06)', color: 'var(--text-muted)' }}>Dismiss</button>
          </>
        ) : (
          <>
      ...
      <div className="px-2.5 py-2" style={{ borderTop: '1px solid var(--border)' }}>
        <p className="text-[10px] line-clamp-2" style={{ color: 'var(--text-muted)' }}>{pv.prompt}</p>
        <div className="flex items-center gap-1.5 mt-1">
          <span className="text-[9px] px-1.5 py-0.5 rounded" style={{ background: 'rgba(79,110,247,0.12)', color: 'var(--accent)' }}>{pv.model}</span>
          <span className="text-[9px]" style={{ color: 'var(--text-muted)' }}>{pv.duration}s{pv.aspectRatio ? ` · ${pv.aspectRatio}` : ''}</span>
        </div>
      </div>
    </div>
  )
...
function VideoCardModal({ item, onClose, onRefresh, onSelectItem, onContinue, promptTemplates, setPromptTemplates, onOpenTemplates, onOpenSource, onTranslate }: {
  ...
  onTranslate?: (params: { sourceItem: VideoItem; langCode: string; langName: string }) => void
}) {
  ...
  const translateUnits = Math.round(selectedLangs.size * (Number(item.duration) || 5) * OMNI_TRANSLATE_RATE)
  function handleTranslate() {
    Array.from(selectedLangs).forEach(code => {
      const name = TRANSLATE_LANGUAGES.find(l => l[0] === code)?.[1] || code
      onTranslate?.({ sourceItem: item, langCode: code, langName: name })
    })
    onClose()
  }
  const [contStatus, setContStatus] = useState<'idle'|'processing'|'done'|'error'>('idle')
  const [contError, setContError] = useState('')
  const [contMixing, setContMixing] = useState(false)
  const contPollRef = useRef<ReturnType<typeof setInterval>|null>(null)
  // Prompt templates в режиме continue
  const contMatchedTemplate = findTemplateInPrompt(contPrompt, promptTemplates)

  function stopContPoll() { if (contPollRef.current) { clearInterval(contPollRef.current); contPollRef.current = null } }

  async function handleContMix() {
    if (!contMatchedTemplate || contMixing) return
    setContMixing(true)
    setQueueActive('openai', true)
    try {
      const cleanUser = stripCommand(contPrompt, contMatchedTemplate.command)
      const res = await fetch('/api/generator/mix', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userPrompt: cleanUser, templateBody: contMatchedTemplate.body }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Mix failed')
      setContPrompt(data.mixed)
      fetch('/api/prompt-templates', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: contMatchedTemplate.id, incrementUse: true }) })
        .then(() => setPromptTemplates(prev => prev.map(t => t.id === contMatchedTemplate.id ? { ...t, uses: t.uses + 1 } : t)))
        .catch(() => {})
    } catch (e: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6"
      ...
      <div className="relative flex rounded-2xl overflow-hidden max-h-[90vh] w-full max-w-3xl"
        ...
        <div className="flex flex-col flex-1 min-w-0 overflow-y-auto p-5">
          ...
          {/* Ссылка на исходное видео (если это продолжение) */}
          {item.sourceVideoId && onOpenSource && (
            <div className="mb-4">
              <button onClick={() => onOpenSource(item.sourceVideoId!)}
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              {/* Prompt templates для режима continue */}
              <div className="flex items-center gap-1">
                <TemplatesButton onClick={onOpenTemplates} />
                {contMatchedTemplate && <MixButton mixing={contMixing} command={contMatchedTemplate.command} onClick={handleContMix} />}
              </div>
            </div>
            ...
            {/* ── Перевод: доступен только для видео со звуком (иначе нечего переводить) ── */}
            {sourceHasAudio && onTranslate && (
              <div className="mb-2 rounded-lg" style={{ border: '1px solid var(--border)' }}>
                <button onClick={() => setTranslatePanelOpen(o => !o)}
                  ...
                  <span className="text-[11px] font-medium" style={{ color: 'var(--text)' }}>
                    🌐 Translate to{selectedLangs.size ? ` · ${selectedLangs.size}` : ''}
                  </span>
                  <svg width="11" height="11" viewBox="0 0 12 12" fill="none" className="transition-transform"
                    style={{ transform: translatePanelOpen ? 'rotate(180deg)' : 'none', color: 'var(--text-muted)' }}>
                ...
                {translatePanelOpen && (
                  <div className="flex flex-wrap gap-1 p-2 pt-0">
                    {TRANSLATE_LANGUAGES.map(([code, name]) => {
                      const on = selectedLangs.has(code)
                      return (
                        <button key={code} onClick={() => toggleLang(code)} title={name}
                          className="px-2 py-1 rounded-md text-[10px] font-mono font-medium transition-all"
                          style={{ background: on ? 'rgba(79,110,247,0.2)' : 'rgba(255,255,255,0.04)', color: on ? 'var(--accent)' : 'var(--text-muted)', border: `1px solid ${on ? 'var(--accent)' : 'var(--border)'}` }}>
                          {code}
                        </button>
                      )
                    })}
            ...
            {selectedLangs.size > 0 ? (
              <button onClick={handleTranslate}
                ...
                🌐 Translate ({selectedLangs.size}) · {translateUnits} units
              </button>
            ) : (
...
export function VideoPage() {
  ...
  // Полный первый кадр — сохраняем оригинал в Drive для скачивания из карточки
  const firstFrameFullRef = useRef<string | null>(null)
  useEffect(() => {
    if (!firstFrame) { firstFrameThumbRef.current = null; firstFrameFullRef.current = null; return }
    firstFrameFullRef.current = `data:image/jpeg;base64,${firstFrame}`
  ...
  // Несколько параллельных поллингов — по одному интервалу на задачу (ключ = procId)
  ...
  // ── Processing videos (временные, в памяти) ──
  const [processingVideos, setProcessingVideos] = useState<ProcessingVideo[]>([])

  function addProcessingVideo(prompt: string, model: string, duration: number, aspectRatio: string): string {
    const id = `proc_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
    setProcessingVideos(prev => [...prev, {
      id, prompt, model, duration, aspectRatio,
      startedAt: Date.now(),
      status: 'processing'
    }])
    return id
  ...
  function removeProcessingVideo(id: string) {
    setProcessingVideos(prev => prev.filter(v => v.id !== id))
  }

  function setProcessingError(id: string, error: string) {
    setProcessingVideos(prev => prev.map(v => v.id === id ? { ...v, status: 'error', error } : v))
  }
  ...
  const canGenerate = useMemo(() => {
    // Генерации идут параллельно и отображаются карточками — глобального блока по статусу нет
  ...
  // ── Polling (многозадачный + устойчивый к перезагрузке) ──
  const PENDING_KEY = 'gen_vid_pending'
  function loadPending(): any[] { try { return JSON.parse(localStorage.getItem(PENDING_KEY) || '[]') } catch { return [] } }
  function savePendingList(list: any[]) { try { localStorage.setItem(PENDING_KEY, JSON.stringify(list)) } catch {} }
  function addPending(rec: any) { savePendingList([...loadPending().filter((r: any) => r.procId !== rec.procId), rec]) }
  function removePending(procId: string) { savePendingList(loadPending().filter((r: any) => r.procId !== procId)) }

  const stopPoll = useCallback((key: string) => {
    const t = pollTimers.current.get(key)
    if (t) { clearInterval(t); pollTimers.current.delete(key) }
  }, [])

  // startedAtOverride используется при возобновлении задачи после перезагрузки
  const pollStatus = useCallback((taskId: string, type: string, savePayload: any, procId?: string, opts?: { refThumb?: string; refFull?: string; startedAt?: number; persist?: boolean }) => {
    const key = procId || taskId
    stopPoll(key)
    const startedAt = opts?.startedAt ?? Date.now()
    const TIMEOUT_MS = 25 * 60 * 1000
    const refThumbSnap = opts?.refThumb ?? firstFrameThumbRef.current ?? undefined
    const refFullSnap = opts?.refFull ?? firstFrameFullRef.current ?? undefined
    // Сохраняем задачу, чтобы после F5 продолжить отслеживание (не потерять видео)
    if (procId && opts?.persist !== false) {
      addPending({ procId, taskId, type, savePayload, startedAt, refThumb: refThumbSnap })
    }
    const timer = setInterval(async () => {
      if (Date.now() - startedAt > TIMEOUT_MS) {
        stopPoll(key); setQueueActive('kling', false)
        if (procId) { setProcessingError(procId, 'Generation timed out. Please try again.'); removePending(procId) }
        return
      }
      try {
        ...
        if (data.task_status === 'succeed') {
          ...
          if (url) {
            fetch('/api/video/save', {
              method: 'POST', headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ videoUrl: url, klingVideoId: vid, refThumb: refThumbSnap || undefined, refFull: refFullSnap || undefined, ...savePayload }),
            }).then(r => r.json()).then(d => {
              if (d.error) { if (procId) setProcessingError(procId, 'Save failed: ' + d.error) }
              else { if (procId) { removeProcessingVideo(procId); removePending(procId) } fetchHistory() }
            }).catch(e => { if (procId) setProcessingError(procId, 'Save failed: ' + e.message) })
          } else if (procId) {
            setProcessingError(procId, 'No video returned'); removePending(procId)
          }
        } else if (data.task_status === 'failed') {
          stopPoll(key); setQueueActive('kling', false)
          if (procId) { setProcessingError(procId, data.task_status_msg ?? 'Generation failed'); removePending(procId) }
        }
      } catch (e: any) {
  ...
  }, [stopPoll, fetchHistory])

  // Возобновление незавершённых задач после перезагрузки страницы
  useEffect(() => {
    const pend = loadPending()
    if (!pend.length) return
    pend.forEach((rec: any) => {
      setProcessingVideos(prev => prev.some(v => v.id === rec.procId) ? prev : [...prev, {
        id: rec.procId,
        prompt: rec.savePayload?.prompt || 'Video',
        model: rec.savePayload?.model || '',
        duration: Number(rec.savePayload?.duration) || 5,
        aspectRatio: rec.savePayload?.aspectRatio || '',
        startedAt: rec.startedAt || Date.now(),
        status: 'processing',
      }])
      pollStatus(rec.taskId, rec.type, rec.savePayload, rec.procId, { refThumb: rec.refThumb, startedAt: rec.startedAt, persist: false })
    })
  ...
  const handleGenerate = async () => {
    ...
    setStatus('idle'); setVideoUrl(null); setError(null)
    setQueueActive('kling', true)

    // Сразу создаём processing-карточку в истории (со спиннером и таймером)
    const procPrompt = videoMode === 'avatar' ? (prompt || 'Avatar video')
      : videoMode === 'motionControl' ? (prompt || 'Motion control video')
  ...
  async function startContinue(params: { sourceItem: VideoItem; prompt: string; model: 'kling-video-o1' | 'kling-v3-omni'; duration: number; keepSound: boolean }) {
    ...
    setQueueActive('kling', true)
    try {
      const urlRes = await fetch(`/api/video/public-url?id=${encodeURIComponent(sourceItem.id)}`)
      const urlData = await urlRes.json()
      if (!urlData.url) throw new Error(urlData.error || 'Failed to get video URL')
  ...
  // Перевод видео на другой язык: та же сцена/настройки, переведены только реплики.
  // Всё на сервере: ffmpeg достаёт первый+последний кадр, GPT переводит реплики,
  // Omni регенерит со звуком. На каждый язык — своя processing-карточка.
  async function startTranslate(params: { sourceItem: VideoItem; langCode: string; langName: string }) {
    const { sourceItem, langCode, langName } = params
    const dur = Number(sourceItem.duration) || 5
    const procId = addProcessingVideo(`${(sourceItem.prompt || 'Video').slice(0, 50)} → ${langName}`, 'kling-v3-omni', dur, sourceItem.aspectRatio)
    setQueueActive('kling', true)
    try {
      const res = await fetch('/api/video/translate', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ fileId: sourceItem.id, langName, prompt: sourceItem.prompt, duration: sourceItem.duration }),
      })
      const d = await res.json()
      if (!res.ok) throw new Error(d.error || 'Translate failed')
      pollStatus(d.task_id, 'omni-video', {
        prompt: d.translatedPrompt || sourceItem.prompt,
        model: 'kling-v3-omni', duration: d.duration || String(dur),
        aspectRatio: sourceItem.aspectRatio, sound: 'on', inputType: 'translate',
        quality: 'pro', sourceVideoId: sourceItem.id, targetLang: langCode,
      }, procId)
    } catch (e: any) {
      setProcessingError(procId, e.message); setQueueActive('kling', false)
    }
    ...
    <div className="flex flex-1 min-h-0">
      ...
      <div className="flex-shrink-0 flex flex-col overflow-y-auto"
        ...
        <div className="flex flex-col gap-0 p-4">
          ...
          <div className="mt-2 flex justify-center">
            <UsageBadge kind="video" refreshKey={processingVideos.length} />
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-y-auto">
        ...
        <div className="p-5">
          ...
          {historyLoading ? (
          ...
          ) : filteredHistory.length === 0 && processingVideos.length === 0 ? (
            ...
            <>
              <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))' }}>
                {/* Генерящиеся видео — карточки со спиннером и таймером, всегда сверху */}
                {processingVideos.map(pv => (
                  <ProcessingVideoCard key={pv.id} pv={pv} onDismiss={() => removeProcessingVideo(pv.id)} />
                ))}
                {filteredHistory.map((item, idx) => (
                  <VideoCard key={item.id} item={item} featured={idx === 0 && processingVideos.length === 0} onSelect={() => setSelectedItem(item)} />
                ))}
              </div>
      ...
      {selectedItem && <VideoCardModal item={selectedItem} onClose={() => setSelectedItem(null)} onRefresh={fetchHistory} onSelectItem={setSelectedItem} onContinue={startContinue}
        promptTemplates={promptTemplates} setPromptTemplates={setPromptTemplates} onOpenTemplates={() => setTemplatesOpen(true)}
        onOpenSource={(id) => { const src = history.find(v => v.id === id); if (src) setSelectedItem(src) }}
